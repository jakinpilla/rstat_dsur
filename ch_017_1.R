...
#' #### 정리
#' 
#' 네 가지 바탕 척도가 존재한다.(이들은 진정한 하위 구성요소들과 관련이 있을 수도 있고 아닐 수도 있다.)
#' 
#' 인자들의 관계를 해석하는 데는 사각회전이 적합하다.
#' 
#' 인자분석은 어떤 특정한 가설을 검증하기 위한 것이 아니라, 잠재적인 가설을 발견하거나 자료 집합에 존재하는 패턴에 관한 정보를 얻기 위한 것일 뿐이다.
#' 
#' #### 인자분석 결과의 보고
#' 
#' 직교회전(배리맥스)된 23개의 문항에 대해 주성분분석을 수행했다. 카이저-마이어-올킨 측도에 의하면 분석에 대한 표집의 적합성은 $KMO = .93$으로 타당하다. 그리고 개별 문항에 대한 KMO 측도는 모두 \\>.77인데, 이는 허용 하한인 .5보다 훨씬 크다. 바틀렛의 구형성 검정은 $\chi^2(253) = 19334, p < .001$로 유의했다. 이는 문항들의 상관관계가 PCA에 충분할 정도로 크다는 뜻이다. 초기 분석으로 자료의 각 성분에 대한 고유값을 구했다. 고유값이 카이저 기준 1보다 큰 성분은 네 개였으며, 이 네 성분의 조합은 변동의 50.32%를 설명했다. 산비탈 그림은 다소 모호했는데, 변곡점을 어디에 두느냐에 따라 성분을 두 개 유지할 수도 있고 네 개 유지할 수도 있었다. 표본 크기가 크다는 점과 산비탈그림의 수렴 패턴, 그리고 네 성분에 대한 카이저의 기준을 고려해서, 최종 분석에서는 네 개의 성분을 유지했다. 회전 이후의 인자적재값들이 표 17.1에 나와있다. 같은 성분에 몰려 있는 문항들을 볼 때, 성분 1은 컴퓨터에 대한 두려움을, 성분 2는 통계학에 대한 두려움을, 성분 3은 수학에 대한 두려움을, 성분 4는 동료 평가에 대한 걱정을 나타낸다고 할 수 있다.
#' 
#' #### 신뢰도 분석
#' 
#' ##### 신뢰성의 측정
#' 
#' 만일 어떤 한 사람이 같은 설문지를 서로 다른 시점에서 두 번 채웠을 때 그 사람이 두 번 다 같은 점수를 받았다면,, 그 설문지는 신뢰성이 있는 것이다.
#' 
#' 신뢰성은 개별 설문 문항이 설문지 전반에 대해 일관된 결과를 내야 한다는 개념에 기초한다.
#' 
#' 실제 연구에서는 반분신뢰도(split-half reliability)라는 것을 이용해서 설문지의 신뢰성을 평가한다.
#' 
#' 두 절반의 상관계수가 반분신뢰도 방법의 검정통계량이다. 이 상관계수의 크기가 클수록 신뢰성이 있을 가능성이 크다.
#' 
#' 하나의 자료 집합을 둘로 나누는 방법은 여러가지이며, 어떻게 나누느냐에 따라 결과가 달라진다. 이 문제를 극복하기 위해 모든 가능한 방식으로 분할한 후 각 분할의 상관계수를 구하고, 그런 상관계수들의 평균을 구한 크론바흐 알파를 측도로 삼는다. 
#' 
...
stasticsFear <- raqData[, c(1, 3, 4, 5, 12, 16, 20, 21)]
mathFear <- raqData[, c(8, 11, 17)]
peerEvaluation <- raqData[, c(2, 9, 19, 22, 23)]


#' statisticsFear의 경우 두 번째 문항인 질문 3은 부정적 점수가 부여되는 문항이므로 -1을 지정해야 한다.
#' 
keys <- c(1, -1, 1, 1, 1, 1, 1, 1)

alpha(computerFear)
alpha(stasticsFear, keys = c(1, -1, 1, 1, 1, 1, 1, 1))
alpha(mathFear)
alpha(peerEvaluation)

#' 신뢰도 분석 결과의 해석
#' 
#' 크론바흐의 알파가 .7에서 .8 사이의 값인지 확인해야 한다.
#' 
#' 만일 `r.drop`의 어떤 값이 .3보다 작다면 해당 문항은 전반적인 척도와 별로 관계가 없는 것이며, 그러면 설문에 문제가 있는 것이다.
#' 
#' 
#' #### 신뢰도분석 결과의 보고
#' 
#' - RAQ의 컴퓨터 두려움, 통계학 두려움, 수학 두려움 하위 척도들은 모두 신뢰도가 높았다. 크론바하의 $\alpha$가 모두 .82이다. 그러나 부정적 동료 평가 두려움 하위척도의 신뢰도는 비교적 낮았다. 크론바흐의 $\alpha$는 .57이다.
#' 
#'  - 신뢰도는 사실 측정의 일관성에 관한 측도이다.
#'  - 신뢰도분석은 설문지의 일관성을 측정하는데 사용할 수 있다.
#'  - 반대 방향 문항을 적절히 처리하는 것을 잊어서는 안 된다. *alpha()* 함수로 분석을 실핼할 때 keys 옵션을 활용해햐 한다.  
#'  - 설문지의 모든 하위 척도에 대해 개별적인 신뢰도분석을 실행한다.
#'  - 크론바하의 $\alpha$는 설문지의 전반적인 신뢰도를 나타낸다. 그 값이 .8(능력 검사의 경우에는 .7) 정도면 좋은 것이다.
#'  - 문항을 제거했을 때의 $\alpha$ 값(*raw_alpha_column_element*)을 보면 해당 문항을 제거했을 때 전반적인 신뢰성이 높아질 것인지를 가늠할 수 있다. 만일 그 $\alpha$값이 전반적인 신뢰도보다 크다면, 그 문항을 제거하면 척도의 전반적인 신뢰도가 높아진다. 제거 시 $\alpha$ 값이 크게 변하는 문장들을 찾아봐야 한다.
#'  - 문장을 제거했다면, 인자분석을 다시 실행해서 인자 구조가 여전히 유효한지 점검해야 한다. 
#'  
#'  정리 
#'  
#'  통계학에서 어려운 점 중에 하나는 통계학 기법들이 사실 주관적이라는 점을 깨닫는 것이다.
#'  
#'  인자분석을 위해서는 먼저 변수들을 조사해서 변수들이 아에 무관하거나 반대로 상관관계가 너무 강하지는 않는지 점검해야 한다. 
#'  
#'  
